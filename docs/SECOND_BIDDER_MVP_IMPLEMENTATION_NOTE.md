...
2. **Eligibility** (“could use” the item) is **not** inferred from gear in this MVP. Pass `eligible_by_loot_id` / `eligible_chars_by_loot_id` into `prepare_second_bidder_events` / `run_from_backup`, or use `run_second_bidder_batch.py` / `run_second_bidder_sample.py` with `--eligibility-json` (see `eligibility_io.py`). If omitted, every attendee passes the eligibility filter.
...
**Current scoring path:** `build_candidate_pool` → `build_feature_bundles` (`compute_capability_raw` with wealth utilization + soft pool caps, `compute_propensity_raw` with win-rate over prior attended loot sales, `compute_competitiveness_raw` with `hoarding_char_lane` and `hoarding_account_total`) → per-event min–max normalization across candidates → `score_candidate` → `normalize_candidate_scores`.
...
**Affected files:** `scripts/second_bidder_model/state.py`, `features.py`, `scoring.py`, `pipeline.py`, `config.py`, `types.py`, `character_plausibility.py`, `eligibility_io.py`, `prepare.py`, `serialize.py`, `debug.py`, `run_second_bidder_batch.py`, `run_second_bidder_sample.py`, tests, spec + this note.
