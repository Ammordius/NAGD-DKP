...
| **`scripts/` (this level)** | **Ledger**: `ledger_delta.py` — diffs two backup dirs and outputs table-level added/removed/changed as HTML (used by CI for the public SQL delta page). **Auth**: `gen_github_worker_jwt.py`. |
